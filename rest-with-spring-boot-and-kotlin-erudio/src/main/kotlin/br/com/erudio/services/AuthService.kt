...
class AuthService {
    ...
    fun signin(data: AccountCredentialsVO) : ResponseEntity<*> {
        logger.info("Tentando logar user ${data.username}")
        return try {
            ...
            throw BadCredentialsException("username ou senha invalidos")
        }
    }

    fun refreshToken(username: String, refreshToken: String) : ResponseEntity<*> {
        logger.info("Tentando fazer um refresh token no user $username")
